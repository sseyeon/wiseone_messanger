...
//    	user.setPassword(Cipher.SHA256Encrypt(user.getPassword()));
		return user;
	}
	...
	public User selectUserByPassWord(Map<String, Object> parameterMap)
			throws Exception {
		User user = null;
    	List<User> list = getSqlMapClientTemplate().queryForList("selectUserByPassWord", parameterMap);
    	if (list.size() > 0) {
    		user = list.get(0);
    	}
		return user;
...
//        parameterMap.put("schema", schema);
        parameterMap.put("locale", locale);
        parameterMap.put("scompany_code", companyCodes);
        return getSqlMapClientTemplate().queryForList("selectDepts", parameterMap);
    }

	@Override
	public List<Dept> selectDepts(String locale, List<String> compCodes) throws Exception {
		Map<String, Object> parameterMap = new HashMap();
		parameterMap.put("locale", locale);
		parameterMap.put("compCodes", compCodes);
		return getSqlMapClientTemplate().queryForList("selectDepts", parameterMap);
	}
...
//        parameterMap.put("schema", schema);
        parameterMap.put("locale", locale);
        parameterMap.put("scompanyCode", companyCodes);
